...
export default class Waifu2x {
  ...
  static typeW(model) {
    ...
    for (let l = 0; l < model.length; l++) {
      ...
      let arrayW = []
      layerWeight.forEach(weightForOutputPlane => {
        weightForOutputPlane.forEach(weightMatrix => {
          weightMatrix.forEach(weightVector => {
            weightVector.forEach(w => arrayW.push(w))
          })
        })
      })
